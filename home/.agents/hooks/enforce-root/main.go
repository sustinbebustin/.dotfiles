...
//
// This hook encodes a Claude Code invariant. Codex runs shell commands through a
// unified-exec session where the working directory plausibly does persist, so it
// is deliberately not registered in the Codex hooks config -- see
...
func main() {
	...
	// Bash is the only tool whose input is a shell command. Claude registers
	// this hook with a Bash-only matcher, but nothing in the payload guarantees
	// that, and Codex sends a patch body in the same tool_input.command field
	// for apply_patch -- a patch that happens to add a literal `cd` line would
	// otherwise trip the shell walk below.
	if in.ToolName != "Bash" {
		os.Exit(0)
	}
	...
	if len(violations) == 0 {
		...
		// and staying quiet leaves the harness's own permission flow in charge.
		os.Exit(0)
	}
	hookio.Render(hookName, harness, hookio.Denied(formatReason(violations)))
}
