Generate a `DESIGN.md` file in the project's `.impeccable/` directory (`$CLAUDE_PROJECT_DIR/.impeccable/DESIGN.md`) that captures the current visual design system, so AI agents generating new screens stay on-brand.
...
Write to `$CLAUDE_PROJECT_DIR/.impeccable/DESIGN.md` (create the `.impeccable/` directory if it doesn't exist; update a legacy root/`.agents/context`/`docs` copy in place instead if one already exists). The file opens with the YAML frontmatter staged in Step 2b (schema documented at the top of this reference), then the markdown body using the structure below. Headers must match character-for-character. Optional evocative subtitles (e.g. `## 2. Colors: The Coastal Palette`) are allowed.
...
Write to `$CLAUDE_PROJECT_DIR/.impeccable/DESIGN.md`, same as Scan mode. Use the six-section spec from Scan mode. Populate what the interview answers; leave the rest as honest placeholders. The seed is a scaffold, not a fabricated spec.
