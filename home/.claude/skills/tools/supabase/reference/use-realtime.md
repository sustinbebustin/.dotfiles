...
$$;
```
Signature: `realtime.send(payload jsonb, event text, topic text, is_private boolean)`. This allows us to broadcast to a specific room with any content that is not bound to a table or if you need to send data to public channels. It's also a good way to integrate with other services and extensions.
