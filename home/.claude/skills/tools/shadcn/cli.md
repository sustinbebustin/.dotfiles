...
> **IMPORTANT:** Always run commands using the project's package runner: `pnpm dlx shadcn@latest` or `bunx --bun shadcn@latest`. Check `packageManager` from project context to choose the right one. npx is not installed, so npm projects use `pnpm dlx` too. Examples below use `pnpm dlx shadcn@latest` but substitute the correct runner for the project.
...
Fuzzy search across registries. Also aliased as `pnpm dlx shadcn@latest list`.
...
See [Workflow step 9 in SKILL.md](./SKILL.md#workflow) for the overwrite / partial / merge / skip decision, commands, and the `apply` and base-preservation caveats.
