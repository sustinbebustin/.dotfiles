---
name: shadcn
description: shadcn/ui workflows — adding, composing, styling, and debugging components (including chat interfaces), plus project init, presets, and registries.
...
> **IMPORTANT:** Run all CLI commands using the project's package runner: `pnpm dlx shadcn@latest` or `bunx --bun shadcn@latest` — based on the project's `packageManager`. npx is not installed, so npm projects use `pnpm dlx` too. Examples below use `pnpm dlx shadcn@latest` but substitute the correct runner for the project.
...
9. **Switching presets** — Ask the user first: **overwrite**, **partial**, **merge**, or **skip**?
   ...
   - **Overwrite**: `pnpm dlx shadcn@latest apply <code>`. Overwrites detected components, fonts, and CSS variables. Use when the user hasn't customized components.
   ...
   - **Merge**: `pnpm dlx shadcn@latest init --preset <code> --force --no-reinstall`, then run `pnpm dlx shadcn@latest info` to list installed components, then for each installed component use `--dry-run` and `--diff` to [smart merge](#updating-components) it individually. Use when the user has customized components.
