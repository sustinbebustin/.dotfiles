...
- `go-reviewer` - Go code review with strict idioms and production reliability
- `frontend-reviewer` - Next.js/React/TypeScript frontend review
- `supabase-reviewer` - Supabase/Postgres security and performance review
...
Task({
  ...
  description: "Research file uploads",
  prompt: "Gather comprehensive documentation about file upload best practices"
})
...
// Go implementation
Task({
  subagent_type: "compound-engineering:development:go-developer",
  description: "Go implementation",
  prompt: "Implement the user service following production-grade patterns"
})
```

**All development agents:**
- `go-developer` - Go implementation with production-grade patterns
- `frontend-developer` - Next.js/React/TypeScript frontend implementation
- `supabase-developer` - Supabase/Postgres database implementation
...
for (const file of ["auth.ts", "user.ts", "api-controller.ts", "payment.ts"]) {
  TaskCreate({
    subject: `Review ${file}`,
...
Task({
  ...
  prompt: `
    ...
    Focus on the user controller endpoints.
  `
})
```
...
const filesToReview = [
  ...
  "src/controllers/api/v1/users.ts",
  "src/controllers/api/v1/payments.ts",
  "src/services/payment-processor.ts",
  "src/services/notification-service.ts",
  "src/lib/encryption-helper.ts"
]
