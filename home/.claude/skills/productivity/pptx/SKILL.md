...
`pptxgenjs` is preinstalled — do not run `pnpm add` first; write the script and `require('pptxgenjs')` directly. Only if that require fails: `pnpm add pptxgenjs`. The model knows the API; these are the footguns:
