...
1. **Determine the target repo:**
   ...
   - If the context lists `### Available repos`, call `AskUserQuestion` with those repo names as options, then gather state for the chosen repo with `git -C <chosen> status`, `git -C <chosen> branch --show-current`, `git -C <chosen> log --oneline -5`, `git -C <chosen> diff --staged`, and `git -C <chosen> diff`.
...
6. **Create a pull request** using `gh pr create`. The PR title should also be a conventional-commit subject (release tooling reads this when squash-merging).
