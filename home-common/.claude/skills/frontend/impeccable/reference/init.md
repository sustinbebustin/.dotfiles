...
- **PRODUCT.md** (strategic): lives in the project's `.impeccable/` directory (`$CLAUDE_PROJECT_DIR/.impeccable/PRODUCT.md`). Covers register, target users, product purpose, brand personality, anti-references, strategic design principles. Answers "who/what/why".
- **DESIGN.md** (visual): lives in the project's `.impeccable/` directory (`$CLAUDE_PROJECT_DIR/.impeccable/DESIGN.md`). Covers visual theme, color palette, typography, components, layout. Follows the [DESIGN.md format spec](https://raw.githubusercontent.com/google-labs-code/design.md/main/docs/spec.md). Answers "how it looks".
...
Check what already exists. PRODUCT.md and DESIGN.md live in the project's `.impeccable/` directory (`$CLAUDE_PROJECT_DIR/.impeccable/`); legacy projects may keep them at the project root or under `.agents/context/` or `docs/` (case-insensitive), which the loader still resolves. Read whichever are present with your native file tool. Also note whether `.impeccable/live/config.json` already exists (Step 6 leaves it untouched if so).
...
Write to `$CLAUDE_PROJECT_DIR/.impeccable/PRODUCT.md` (create the `.impeccable/` directory if it doesn't exist). If a legacy `PRODUCT.md` already exists at the project root or under `.agents/context/`/`docs/`, merge into that content in place rather than starting from scratch or duplicating it into `.impeccable/`.
