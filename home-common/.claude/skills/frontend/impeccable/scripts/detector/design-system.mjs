...
// The project's .impeccable/ directory. CLAUDE_PROJECT_DIR (set by Claude Code
// in hook/MCP/LSP contexts) always points at the project root; otherwise walk up
// from cwd to the nearest existing .impeccable/. Recovers the design context when
// the detector runs from a nested package whose git/workspace boundary hides the
// root. Standalone here — the detector is bundled apart from context.mjs.
function resolveImpeccableDir(cwd = process.cwd()) {
  const env = process.env.CLAUDE_PROJECT_DIR;
  if (env && env.trim()) {
    return path.join(path.resolve(env.trim()), IMPECCABLE_DIR);
  }
...
function resolveDesignMdPath(cwd = process.cwd()) {
  ...
  const impeccableDir = resolveImpeccableDir(cwd);
  if (impeccableDir) {
    const found = firstExisting(impeccableDir, DESIGN_NAMES);
    if (found) return { path: found, contextDir: impeccableDir };
  }
...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  ...
  const candidates = [
    ...
    ...(impeccableDir ? [path.join(impeccableDir, "design.json")] : []),
