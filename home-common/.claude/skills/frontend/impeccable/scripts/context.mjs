 ...
 *   2. Active project .impeccable/, then .agents/context/, then docs/
 *   3. Monorepo root context, using the same order, as a per-file fallback
 *   4. $CLAUDE_PROJECT_DIR/.impeccable/ (Claude Code sets CLAUDE_PROJECT_DIR in
 *      hook/MCP/LSP contexts), else the nearest .impeccable/ in cwd or any
 *      ancestor — recovers context when the script runs from a nested package
 *      whose git/workspace boundary hides the real project root
 *   5. $IMPECCABLE_CONTEXT_DIR (absolute or cwd-relative) — power-user
 *      escape hatch, only consulted when defaults are empty
 *   6. Active project root as a "nothing found" default
 *
...
const FALLBACK_DIRS = [IMPECCABLE_DIR, ".agents/context", "docs"];
...
function resolveContext(cwd = process.cwd(), options = {}) {
  ...
  if (!productPath && !designPath) {
    // Recover context when the script runs from a nested package whose git or
    // workspace boundary hides the real project root (resolveProject stops at
    // that boundary, so the local/root lookups above miss a parent .impeccable/).
    const impeccableDir = findContextImpeccableDir(absCwd);
    if (impeccableDir) {
      productPath = firstExisting(impeccableDir, PRODUCT_NAMES);
      designPath = firstExisting(impeccableDir, DESIGN_NAMES);
    }
  }
...
// The absolute project root supplied by Claude Code. Set in the environment of
// hook commands, stdio MCP servers, and plugin LSP servers (see the hooks docs),
// and always the project root — never the script's cwd. Null in plain script or
// Bash-tool runs, where the callers fall back to a cwd-upward .impeccable/ walk.
function envProjectDir() {
  const dir = process.env.CLAUDE_PROJECT_DIR;
  return dir && dir.trim() ? path.resolve(dir.trim()) : null;
}

// The .impeccable/ that actually holds PRODUCT.md / DESIGN.md: CLAUDE_PROJECT_DIR
// first (when it points at a populated .impeccable/), then the nearest populated
// .impeccable/ walking up from cwd. Used for context reads, so it requires the
// files to be present rather than just the directory to exist.
function findContextImpeccableDir(cwd) {
  const projectDir = envProjectDir();
  if (projectDir) {
    const dir = path.join(projectDir, IMPECCABLE_DIR);
    if (firstExisting(dir, [...PRODUCT_NAMES, ...DESIGN_NAMES])) return dir;
  }
  let dir = path.resolve(cwd);
  const homeDir = path.resolve(os.homedir());
  while (true) {
    const candidate = path.join(dir, IMPECCABLE_DIR);
    if (firstExisting(candidate, [...PRODUCT_NAMES, ...DESIGN_NAMES])) {
      return candidate;
    }
...
// The nearest existing .impeccable/ directory in cwd or any ancestor, regardless
// of what it contains. Used for resolving the canonical state directory (design
// sidecar, live-server info, critique, config) rather than the context files.
function findAncestorImpeccableDir(cwd) {
  let dir = path.resolve(cwd);
  const homeDir = path.resolve(os.homedir());
  while (true) {
    const candidate = path.join(dir, IMPECCABLE_DIR);
...
/**
 * The canonical `.impeccable/` directory for a project. Honors Claude Code's
 * CLAUDE_PROJECT_DIR (set in hook/MCP/LSP contexts) so nested-package cwds still
 * resolve to the project root; then an existing `.impeccable/` in cwd or any
 * ancestor; then the resolved project root as the create-here default. Shared
 * with lib/impeccable-paths.mjs so the design sidecar and live state land in the
 * same place the context loader reads from.
 */
export function resolveImpeccableDir(cwd = process.cwd(), options = {}) {
  const projectDir = envProjectDir();
  if (projectDir) return path.join(projectDir, IMPECCABLE_DIR);
  return (
    findAncestorImpeccableDir(cwd) ||
