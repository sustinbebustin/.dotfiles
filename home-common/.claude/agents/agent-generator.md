...
<role>
  ...
  routing logic, context management, and workflow design for Claude Code agents
</role>
...
<inputs_required>
  ...
  <parameter name="domain_analysis" type="object">
    Domain analysis with core concepts and knowledge structure
  </parameter>
...
<process_flow>
  <step_1>
    ...
    <process>
      1. Create frontmatter with Claude Code supported fields only
    ...
    <template_structure>
      ...
      description: {purpose}. Use proactively for {domain} coordination tasks.
  ...
  <step_2>
    ...
    <template_structure>
      ...
      ---
      name: {subagent-name}
      description: {specific task this subagent performs}. Use proactively when {trigger condition}.
      tools: Read, Write, Edit, Glob, Grep
      model: sonnet
      ---
...
<claude_code_frontmatter>
  <supported_fields>
    Claude Code parses these frontmatter fields for agents:
    ...
    - tools: (optional) Comma-separated list (e.g., "Read, Write, Edit, Bash, Glob, Grep")
            If omitted, inherits all tools from main thread
    - model: (optional) Model alias: "sonnet", "opus", "haiku", or "inherit"
            Defaults to configured subagent model if omitted
  ...
  <unsupported_fields>
    Do NOT include these in Claude Code agents:
    - id, category, type, version, author, tags (metadata for other systems)
    - mode (no agent vs subagent distinction in frontmatter)
    - temperature (not configurable per-agent)
    - tools as YAML object (must be comma-separated string)
  </unsupported_fields>
  
  <tool_names>
    Available tools use PascalCase:
    Read, Write, Edit, Bash, Glob, Grep, plus any MCP server tools
  </tool_names>
</claude_code_frontmatter>
...
<constraints>
  ...
  <must>Use only Claude Code supported frontmatter fields (name, description, tools, model, permissionMode, skills)</must>
  <must>Include both name and description in frontmatter (required fields)</must>
  <must>Use comma-separated tool names in PascalCase (e.g., "Read, Write, Edit")</must>
  <must>Use @ symbol for all subagent routing in prompt body</must>
  ...
  <must_not>Include unsupported frontmatter (id, category, type, version, author, tags, mode, temperature)</must_not>
  <must_not>Use tools as YAML object (must be comma-separated string)</must_not>
...
<validation_checks>
  ...
  <post_execution>
    ...
    - Tools specified as comma-separated PascalCase strings
...
<generation_principles>
  ...
  <claude_code_native>
    Use only features and frontmatter supported by Claude Code
  </claude_code_native>
</generation_principles>