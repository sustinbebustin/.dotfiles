 ...
 *
 * Runs tsc + qlty after Edit/Write on .ts/.tsx files.
 * Warns on errors (non-blocking) so multi-file refactors can proceed.
 * The Stop hook (typescript-stop-gate) enforces that all errors are resolved
...
async function main() {
  try {
    ...
    let scriptPath = path.join(projectDir, '.claude', 'hooks', 'typescript_check.py');
    if (!fs.existsSync(scriptPath)) {
      scriptPath = path.join(homeDir, '.claude', 'hooks', 'typescript_check.py');
    }
    if (!fs.existsSync(scriptPath)) {
    ...
    try {
      ...
      if (checkResult.has_errors) {
        // Format errors as a warning (non-blocking)
        const errorLines: string[] = [];
        errorLines.push(`[WARN] TypeScript Pre-flight: ${checkResult.summary}`);
        errorLines.push('');
        ...
        errorLines.push('');
        errorLines.push('These must be resolved before the task is complete.');

        // Non-blocking: log to stderr so it appears in output but does not block
      ...
      if (checkError && typeof checkError === 'object' && 'status' in checkError) {
        ...
        if (execError.stdout) {
          try {
            ...
            if (checkResult.has_errors) {
              process.stderr.write(`[WARN] TypeScript Pre-flight: ${checkResult.summary}\n`);
              console.log(JSON.stringify({}));
              return;
            }
          } catch {
