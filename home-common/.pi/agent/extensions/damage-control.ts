...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const globalRulesPath = path.join(extensionDir, "..", "damage-control-rules.yaml");
		const projectRulesPath = path.join(ctx.cwd, ".pi", "damage-control-rules.yaml");
		const rulesPath = fs.existsSync(projectRulesPath) ? projectRulesPath : globalRulesPath;
		try {
			if (fs.existsSync(rulesPath)) {
				...
				ctx.ui.notify(`Damage-Control: No rules found at ${rulesPath}`);
			}
		} catch (err) {
